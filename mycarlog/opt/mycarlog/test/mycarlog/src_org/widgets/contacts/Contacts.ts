...
export default class Contacts extends WidgetBase<ContactsProperties> {
  ...
  async onAttach() {
    this.idaccount = window.GlobalStore.get(
      window.GlobalStore.path("root", "user", "idaccount")
    );

    const res = await fetch("/contacts_r", {
      method: "POST",
      body: JSON.stringify({ idaccount: this.idaccount }),
      headers: {
        ...
        w(IconX, {
          ...
          onClick: (e) => {
            window.location.href =
              "/#contact?idcontact=0&rowkey=0&idaccount=" + this.idaccount;
          },
        }),
      ]),
      v("div", {}, [
